...
function dashboard_widgets_suite_enable_widgets() {
	...
	if (isset($dws_options_general['widget_control_panel']) && $dws_options_general['widget_control_panel']) {
		...
		if (dashboard_widgets_suite_check_role($display_control_panel)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_control_panel', esc_html__('Control Panel', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_control_panel');
	...
	if (isset($dws_options_log_debug['widget_log_debug']) && $dws_options_log_debug['widget_log_debug']) {
		...
		if (dashboard_widgets_suite_check_role($display_log_debug)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_log_debug', esc_html__('Debug Log', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_log_debug');
	...
	if (isset($dws_options_log_error['widget_log_error']) && $dws_options_log_error['widget_log_error']) {
		...
		if (dashboard_widgets_suite_check_role($display_log_error)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_log_error', esc_html__('Error Log', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_log_error');
	...
	if (isset($dws_options_feed_box['widget_feed_box']) && $dws_options_feed_box['widget_feed_box']) {
		...
		if (dashboard_widgets_suite_check_role($display_feed_box)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_feed_box', esc_html__('Feed Box', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_feed_box');
	...
	if (isset($dws_options_social_box['widget_social_box']) && $dws_options_social_box['widget_social_box']) {
		...
		if (dashboard_widgets_suite_check_role($display_social_box)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_social_box', esc_html__('Social Box', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_social_box');
	...
	if (isset($dws_options_list_box['widget_list_box']) && $dws_options_list_box['widget_list_box']) {
		...
		if (dashboard_widgets_suite_check_role($display_list_box)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_list_box', esc_html__('List Box', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_list_box');
	...
	if (isset($dws_options_widget_box['widget_widget_box']) && $dws_options_widget_box['widget_widget_box']) {
		...
		if (dashboard_widgets_suite_check_role($display_widget_box)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_widget_box', esc_html__('Widget Box', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_widget_box');
	...
	if (isset($dws_options_system_info['widget_system_info']) && $dws_options_system_info['widget_system_info']) {
		...
		if (dashboard_widgets_suite_check_role($display_system_info)) {
			...
			wp_add_dashboard_widget('dashboard_widgets_suite_system_info', esc_html__('System Info', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_system_info');
	...
	if (isset($dws_options_notes_user['widget_notes_user']) && $dws_options_notes_user['widget_notes_user']) {
		...
		wp_add_dashboard_widget('dashboard_widgets_suite_notes_user', esc_html__('User Notes', 'dashboard-widgets-suite') . $suite, 'dashboard_widgets_suite_notes_user');
